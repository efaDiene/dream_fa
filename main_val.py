...
class GUI:
    def __init__(self, opt):
        ...
        self.T116 = np.array([[-0.7255133758153133, 0.16008781379961542,-0.669329689604404,  -3.5211266181713743],
        ...
        self.T21 = np.array([[0.9958024195334342,-0.018871573594701076,0.0895622965394354,1.0876262766729021],
    ...
    def prepare_train(self):
        ...
        self.azimuth, self.elevation =self.extract_azimuth_elevation(self.T21)
        t = self.T21[:3, 3]
        self.rayon = np.linalg.norm(t)
        pose = orbit_camera(self.elevation, self.azimuth, 2 )
        print("1",self.azimuth, self.elevation)
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            if self.input_img_torch is not None and not self.opt.imagedream:
                ...
                image4 = image.squeeze(0)  # Passer de [1, 3, H, W] à [3, H, W]
                image4 = image4.permute(1, 2, 0)  # Passer de [3, H, W] à [H, W, 3]

                # Assurez-vous que les valeurs sont entre 0 et 255 (pour l'image RGB)
                image4 = (image4 * 255).clamp(0, 255).byte()  # Normaliser et convertir en entier

                # Enregistrer l'image avec OpenCV
                cv2.imwrite('output_image_train.jpg', image4.cpu().numpy())
                
                image3 = self.input_img_torch.squeeze(0)  # Passer de [1, 3, H, W] à [3, H, W]
                image3 = image3.permute(1, 2, 0)  # Passer de [3, H, W] à [H, W, 3]

                # Assurez-vous que les valeurs sont entre 0 et 255 (pour l'image RGB)
                image3 = (image3 * 255).clamp(0, 255).byte()  # Normaliser et convertir en entier

                # Enregistrer l'image avec OpenCV
                cv2.imwrite('output_image_trainT.jpg', image3.cpu().numpy())
    ...
    def val_step(self):
        with torch.no_grad():
            ...
            #self.optimizer = self.renderer.gaussians.optimizer
            t = self.T116[:3, 3]
            self.rayon2 = np.linalg.norm(t)
            #print(self.rayon2)
            azimuth2, elevation2 =self.extract_azimuth_elevation(self.T116)
            # default camera
            #self.cam.scale(t)
            pose2 = orbit_camera( elevation2 , azimuth2+5 , self.opt.radius)
            #print("2",azimuth2, elevation2 )
            ...
            """  self.enable_zero123 = self.opt.lambda_zero123 > 0 and self.input_img2 is not None
            if self.guidance_zero123 is None and self.enable_zero123:
                from guidance.zero123_utils import Zero123
                if self.opt.stable_zero123:
                    self.guidance_zero123 = Zero123(self.device, model_key='ashawkey/stable-zero123-diffusers')
            ...
            if self.input_img2 is not None:
                ...
                #self.input_img_torch2 = F.interpolate(self.input_img_torch2, (self.opt.ref_size, self.opt.ref_size), mode="bilinear", align_corners=True)

                self.input_mask_torch2 = torch.from_numpy(self.input_mask2).permute(2, 0, 1).unsqueeze(0).to(self.device)
                #self.input_mask_torch2 = F.interpolate(self.input_mask_torch2, (self.opt.ref_size, self.opt.ref_size), mode="bilinear", align_corners=True)
            ...
            for _ in range(self.train_steps):
                ...
                ### known view
                if self.input_img_torch2 is not None and not self.opt.imagedream:
                    cur_cam = self.fixed_cam2
                    out = self.renderer.render(cur_cam)

                    # rgb loss
                    image_v = out["image"].unsqueeze(0) # [1, 3, H, W] in [0, 1]
                    loss_v = loss_v + F.mse_loss(image_v, self.input_img_torch2)
                    self.rgb_val_loss.append((F.mse_loss(image_v, self.input_img_torch2)).cpu())

                    # mask loss
                    mask = out["alpha"].unsqueeze(0) # [1, 1, H, W] in [0, 1]
                    loss_v = loss_v + F.mse_loss(mask, self.input_mask_torch2)
                    self.mask_val_loss.append((F.mse_loss(mask, self.input_mask_torch2)).cpu())
                    image2 = image_v.squeeze(0)  # Passer de [1, 3, H, W] à [3, H, W]
                    image2 = image2.permute(1, 2, 0)  # Passer de [3, H, W] à [H, W, 3]
                    ...
                    # Enregistrer l'image avec OpenCV
                    cv2.imwrite('output_image_val.jpg', image2.cpu().numpy())
                    image3 = self.input_img_torch2.squeeze(0)  # Passer de [1, 3, H, W] à [3, H, W]
                    image3 = image3.permute(1, 2, 0)  # Passer de [3, H, W] à [H, W, 3]
                    ...
                    # Enregistrer l'image avec OpenCV
                    cv2.imwrite('output_image_valT.jpg', image3.cpu().numpy())
                    print(loss_v)
    ...
    def train(self, iters=500):
        ...
        # save
        epochs = range(1, len(self.rgb_val_loss) + 1)

        # Création de la figure
        plt.figure(figsize=(10, 6))
        plt.plot(epochs, self.rgb_val_loss, label='RGB Val Loss', marker='o')
        plt.plot(epochs, self.mask_val_loss, label='Mask Val Loss', marker='s')

        # Personnalisation
        plt.title('Evolution des pertes (Loss) de validation au cours des époques')
        plt.xlabel('Époques')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True)
        plt.savefig("loss_val_plot.png", dpi=300, bbox_inches='tight') 

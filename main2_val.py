...
class GUI:
    ...
    def prepare_train(self):
        ...
        pose = orbit_camera(self.elevation, self.azimuth, self.opt.radius)
    ...
    def val_step(self):
        with torch.no_grad():
            ...
            # default camera
            pose2 = orbit_camera( elevation2 + self.elevation, azimuth2 + self.azimuth, 2)
            ...
            if self.input_img2 is not None:
                ...
                self.input_mask_torch2 = torch.from_numpy(self.input_mask).permute(2, 0, 1).unsqueeze(0).to(self.device)
            ...
            for _ in range(self.train_steps):
                ...
                if self.input_img_torch2 is not None and not self.opt.imagedream:
                    ...
                    ssaa = min(2.0, max(0.125, 2 * np.random.random()))
                    out = self.renderer.render(*self.fixed_cam2, self.opt.ref_size, self.opt.ref_size2, ssaa=ssaa)

                    # rgb loss
                    image_v = out["image"] # [H, W, 3] in [0, 1]
                    valid_mask = ((out["alpha"] > 0) & (out["viewcos"] > 0.5)).detach()
                    loss_v = loss_v + F.mse_loss(image_v * valid_mask, self.input_img_torch_channel_last2 * valid_mask)
                    print(loss_v)
                    self.rgb_val_loss.append(loss_v.cpu())
                    ...
                    # Enregistrer l'image avec OpenCV
                    cv2.imwrite('output_image_val2.jpg', image2.cpu().numpy())
    ...
    def train(self, iters=500):
        ...
        # Personnalisation
        plt.title('Evolution des pertes (Loss2) de validation au cours des époques')
        plt.xlabel('Époques')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True)
        plt.savefig("loss_val_plot2.png", dpi=300, bbox_inches='tight') 
